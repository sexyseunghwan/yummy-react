...
export async function fetchAndRenderMarker(
    ...
    //     sw: bounds.getSW(), /* 지도 남서쪽 좌표 */
    //     ne: bounds.getNE(), /* 지도 북동쪽 좌표 */
    //     center: bounds.getCenter(), /* 지도 정중앙 좌표 */
    //     zoom: zoom
    // };
    ...
    if (mapContext.showOnlyZeroPayPrevRef.current === mapContext.showOnlyZeroPayRef.current) {
        ...
        //deleteAllMarkers(markerMap, markers);
    }
    ...
    // } else {
    //     /* api 호출 */ 
    //     stores = await fetchStores(mapContext.apiBaseUrl, mapParams, showOnlyZeroPay);
 ...
 * @param markers 
 */
// function deleteAllMarkers(markerMap: Map<number, any>, markers: any[]) {
//     for (const [seq, marker] of markerMap.entries()) {
//         naver.maps.Event.clearInstanceListeners(marker); /* 리스너 제거 */
//         markerMap.delete(seq); /* Map 에서 제거 */ 
//         marker.setMap(null); /* 지도에서 제거 */ 
